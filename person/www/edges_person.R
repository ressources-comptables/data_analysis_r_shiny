...
process_edges_person <- function(data) {

  # 1. Create Edges Dataframe
  # ----------------------------------------
  
  
  # 1.1) Emitter to Person_name Edges
  # ----------------------------------------
  
  # Filter out rows where Emitter or Person_name is NA
  valid_data <- data %>%
    filter(!is.na(Emitter) & !is.na(Person_name))
  
  # Group by Emitter and Person_name to calculate the number of distinct Line_id and sum of Amount_converted
  edges_emitter_person <- valid_data %>%
    group_by(Emitter, Person_name) %>%
    summarise(
  ...
  # 1.2) Person_name to Person_name Edges
  # ----------------------------------------
  
  # Step 1: Filter rows with distinct Line_id having more than 1 distinct Person_name
  filtered_data <- data %>%
    filter(!is.na(Line_id) & !is.na(Person_name)) %>%
    group_by(Line_id) %>%
    filter(n_distinct(Person_name) > 1) %>%
    ungroup()
  
  # Check if filtered_data is empty (if not it will not work when there are no person-person edges)
  if (nrow(filtered_data) > 0) {
    
    # Step 2: Construct Person_name pairs
    person_pairs <- filtered_data %>%
      select(Line_id, Person_name, Amount_converted) %>%
      group_by(Line_id) %>%
      summarise(pairs = list(combn(Person_name, 2, simplify = FALSE)), Amount_converted = first(Amount_converted)) %>%
      unnest(pairs) %>%
